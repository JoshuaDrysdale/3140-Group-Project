...
function App() {
  ...
  return (
    <BrowserRouter>
      {user && <Navbar user={user} cartCount={totalItems} onCartClick={() => setCartOpen(!cartOpen)} onLogout={() => setUser(null)} />}
      {cartOpen && <Cart cart={cart} onAdd={addToCart} onRemove={removeFromCart} onClose={() => setCartOpen(false)} />}
      <Routes>
        <Route path="/" element={user ? <Navigate to="/store" /> : <Login onLogin={setUser} />} />
        <Route path="/store" element={user ? <Store /> : <Navigate to="/" />} />
        <Route path="/category/:category" element={user ? <CategoryPage onAddToCart={addToCart} cart={cart} /> : <Navigate to="/" />} />
      </Routes>
    </BrowserRouter>
  );
}
